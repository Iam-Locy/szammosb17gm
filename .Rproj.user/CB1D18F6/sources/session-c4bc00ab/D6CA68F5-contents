...
n.integrate <- function(time=time,init.x= init.x,model=lvm){
...
# Note: the default palette contains only 8 colors, which are recycled after the eigth index; see palette()
plot.lvm.time <- function(out){
        n <- length(out[1,])-1
        colors <- rainbow(n) # pick equidistant colors from the "rainbow"
	t.range <- range(out[,1])
        y.range <- range(out[,2:(n+1)])
        plot(t.range,y.range,xlab="time",ylab="abundance",type="n")
        for (i in c(2:(n+1))) {lines(out[,1],out[,i],col=colors[i-1])}
}
...
plot.matrix <- function(matrix,index){
         ...
         m[index,] <-0
         m[,index] <-0
         image(m,axes=F,zlim=c(0,1),col=gray(c(32:0)/32),xlab="species affected",ylab="species affecting")
